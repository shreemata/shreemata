...
function displayCommissions(commissions) {
    ...
    commissions.forEach(commission => {
        ...
        const typeClass = commission.commissionType === 'direct' ? 'type-direct' : 
                          commission.commissionType === 'referral' ? 'type-referral' : 'type-tree';
        const typeText = commission.commissionType === 'direct' ? 'Direct Commission (Cashback)' : 
                         commission.commissionType === 'referral' ? 'Referral' : 'Tree';
        const typeIcon = commission.commissionType === 'direct' ? '💰' : 
                         commission.commissionType === 'referral' ? '👥' : '🌳';

        const levelText = (commission.commissionType === 'direct' || commission.commissionType === 'referral') ? 'L1' : `L${commission.level}`;
