...
// Pre-save hook: Enforce exactly one admin account system-wide
userSchema.pre('save', async function(next) {
  if (this.role === 'admin' && (this.isNew || this.isModified('role'))) {
    ...
    if (existingAdminCount >= 1) {
      return next(new Error("Only one admin account is allowed. Remove or demote the existing admin first before assigning admin access to another account."));
    }
  }
  next();
});

// Pre-update hooks: Enforce single admin on update operations
userSchema.pre(['updateOne', 'findOneAndUpdate', 'updateMany'], async function(next) {
  ...
  if (isSettingAdmin) {
    ...
    const targetUser = await UserModel.findOne(filter);
    const targetId = targetUser ? targetUser._id : null;
