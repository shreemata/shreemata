...
// GET USER VIP MASTER CARDS MILESTONES
router.get("/users/profile/vip-mastercards", authenticateToken, async (req, res) => {
  try {
...
/* -------------------------------------------
   POST /api/users/:id/master-card
   Generate and assign a Master Card (Admin Only)
--------------------------------------------*/
router.post("/users/:id/master-card", authenticateToken, async (req, res) => {
  try {
    ...
    if (user.masterCard && user.masterCard.isAssigned) {
      return res.status(400).json({ error: "User already has a Master Card assigned" });
    }

    // Direct referrals count check (must be > 20)
    const referralCount = user.referralCode ? await User.countDocuments({ referredBy: user.referralCode }) : 0;
    if (referralCount <= 20) {
      return res.status(400).json({ error: "User must have more than 20 direct referrals to unlock the Master Card." });
    }

    // Generate unique card number SMC-XXXXX
    const lastUser = await User.findOne({ 'masterCard.cardNumber': { $regex: /^SMC-\d+$/ } })
      .sort({ 'masterCard.cardNumber': -1 });
    
    let nextNum = 10001;
    if (lastUser && lastUser.masterCard && lastUser.masterCard.cardNumber) {
      const parts = lastUser.masterCard.cardNumber.split('-');
      if (parts.length === 2) {
    ...
    await user.save();
    console.log(`💳 Assigned Master Card ${cardNumber} to user ${user.email}`);

    res.json({
      message: "Master Card generated successfully",
      masterCard: user.masterCard
    });

  } catch (err) {
    console.error("GENERATE MASTER CARD ERROR:", err);
    res.status(500).json({ error: "Server error generating Master Card" });
  }
});

/* -------------------------------------------
   GET /api/users/:id/master-card
   Fetch Master Card details (Admin or specific owner only)
--------------------------------------------*/
router.get("/users/:id/master-card", authenticateToken, async (req, res) => {
  try {
    const requesterId = req.user.id.toString();
    const targetUserId = req.params.id;

    // Security Check: requester must be Admin OR the owner themselves
    if (req.user.role !== 'admin' && requesterId !== targetUserId) {
      return res.status(403).json({ error: "Forbidden: You do not have permission to access this Master Card" });
    }
    ...
    if (!user.masterCard || !user.masterCard.isAssigned) {
      return res.status(404).json({ error: "No Master Card assigned to this user" });
    }
    ...
    console.error("FETCH MASTER CARD ERROR:", err);
    res.status(500).json({ error: "Server error fetching Master Card" });
  }
});
