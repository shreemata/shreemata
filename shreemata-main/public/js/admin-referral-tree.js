const API = window.API_URL || (window.location.hostname === 'localhost' ? 'http://localhost:3000/api' : '/api');
...
/* -------------------------------------------------------------
   MANUAL TREE PLACEMENT TOOL
   ------------------------------------------------------------- */
let selectedUserToMove = null;
let selectedNewParent = null;
let selectedPositionSlot = null;

function initManualPlacementTool() {
    const searchUserToPlaceInput = document.getElementById('searchUserToPlace');
    const userToPlaceSearchResults = document.getElementById('userToPlaceSearchResults');
    const searchParentUserInput = document.getElementById('searchParentUser');
    const parentSearchResults = document.getElementById('parentSearchResults');
    const placeAtRootToggle = document.getElementById('placeAtRootToggle');
    const parentSearchContainer = document.getElementById('parentSearchContainer');
    const confirmBtn = document.getElementById('confirmManualPlacementBtn');
    
    if (!searchUserToPlaceInput) return;
    
    // User search input
    setupSearchAutocomplete(searchUserToPlaceInput, userToPlaceSearchResults, (user) => {
        selectedUserToMove = user;
        displaySelectedUserToMove(user);
        updateStep3Visibility();
    });
    
    // Parent search input
    setupSearchAutocomplete(searchParentUserInput, parentSearchResults, (parent) => {
        selectedNewParent = parent;
        displaySelectedParent(parent);
        loadSlotsForParent(parent._id);
    });
    
    // Toggle Place at Root
    placeAtRootToggle.addEventListener('change', (e) => {
        if (e.target.checked) {
            parentSearchContainer.style.display = 'none';
            document.getElementById('selectedParentDetails').style.display = 'none';
            selectedNewParent = { _id: 'root', name: 'Root (No Parent)' };
            loadSlotsForParent('root');
        } else {
            parentSearchContainer.style.display = 'block';
            selectedNewParent = null;
            document.getElementById('selectedParentDetails').style.display = 'none';
            document.getElementById('positionAndConfirmContainer').style.display = 'none';
        }
    });
    
    confirmBtn.addEventListener('click', handleManualPlacementSubmit);
}

function setupSearchAutocomplete(inputElement, resultsElement, onSelectCallback) {
    let debounceTimer;
    ...
    inputElement.addEventListener('input', (e) => {
        ...
        debounceTimer = setTimeout(async () => {
            try {
                ...
                const data = await response.json();
                renderSearchResults(data.users, resultsElement, (user) => {
                    inputElement.value = '';
                    resultsElement.style.display = 'none';
                    onSelectCallback(user);
                });
            } catch (err) {
                console.error('Error during user autocomplete:', err);
            }
        }, 300);
    });
...
function renderSearchResults(users, resultsElement, onSelect) {
    resultsElement.innerHTML = '';
    
    if (!users || users.length === 0) {
        resultsElement.innerHTML = '<div style="padding: 10px; color: #666; font-size: 13px;">No users found</div>';
    ...
    users.forEach(user => {
        ...
        item.style.cssText = 'padding: 10px 12px; cursor: pointer; border-bottom: 1px solid #f1f5f9; font-size: 13.5px;';
        
        item.innerHTML = `
            <div style="font-weight: 600; color: #1e293b;">${user.name}</div>
            <div style="font-size: 12px; color: #64748b;">${user.email} | Code: ${user.referralCode || 'None'}</div>
        `;
...
function displaySelectedUserToMove(user) {
    const detailsDiv = document.getElementById('selectedUserToPlaceDetails');
    
    detailsDiv.innerHTML = `
        <div style="font-weight: 600; color: #667eea; margin-bottom: 4px;">Selected User:</div>
        <strong>Name:</strong> ${user.name}<br>
        <strong>Email:</strong> ${user.email}<br>
        <strong>Current Tree Placement:</strong> Level ${user.treeLevel || 0}, Position ${user.treePosition || 0}<br>
        <strong>Direct Referrer Code:</strong> ${user.referredBy || 'None'}
    `;
    detailsDiv.style.display = 'block';
}

function displaySelectedParent(parent) {
    const detailsDiv = document.getElementById('selectedParentDetails');
    detailsDiv.innerHTML = `
        <div style="font-weight: 600; color: #10b981; margin-bottom: 4px;">Selected Parent:</div>
        <strong>Name:</strong> ${parent.name}<br>
        <strong>Email:</strong> ${parent.email}<br>
        <strong>Tree Level:</strong> Level ${parent.treeLevel || 0}
    `;
    detailsDiv.style.display = 'block';
}

async function loadSlotsForParent(parentId) {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${API}/admin/referral-tree/parents/${parentId}/slots`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        
        if (!response.ok) throw new Error('Failed to fetch parent slot info');
        
        const data = await response.json();
        renderPositionSlots(data.slots);
        updateStep3Visibility();
    } catch (err) {
        console.error('Error loading parent slots:', err);
        alert('Error loading parent slots configuration');
    }
}

function renderPositionSlots(slots) {
    const listDiv = document.getElementById('positionSlotsList');
    listDiv.innerHTML = '';
    ...
    slots.forEach(slot => {
        ...
        card.style.cssText = `
            border: 2px solid ${isOccupied ? '#f1f5f9' : '#cbd5e1'};
            background: ${isOccupied ? '#f1f5f9' : 'white'};
            padding: 12px;
            border-radius: 6px;
        ...
        card.innerHTML = `
            <div style="font-weight: 700; font-size: 14px; color: ${isOccupied ? '#64748b' : '#334155'};">Slot ${slot.position}</div>
            <div style="font-size: 11px; margin-top: 4px; color: ${isOccupied ? '#ef4444' : '#10b981'}; font-weight: 600;">
                ${isOccupied ? 'OCCUPIED' : 'FREE'}
            </div>
            ${isOccupied ? `<div style="font-size: 11px; color: #64748b; margin-top: 4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${slot.occupant.name}">${slot.occupant.name}</div>` : ''}
        `;
        
        if (!isOccupied) {
            card.addEventListener('mouseenter', () => {
                if (selectedPositionSlot !== slot.position) {
            ...
            card.addEventListener('click', () => {
                selectedPositionSlot = slot.position;
                
                Array.from(listDiv.children).forEach((child, index) => {
                    const otherSlot = slots[index];
                    if (!otherSlot.occupant) {
                ...
                card.style.borderColor = '#f59e0b';
                card.style.boxShadow = '0 0 0 3px rgba(245, 158, 11, 0.2)';
            });
        }
...
function updateStep3Visibility() {
    const container = document.getElementById('positionAndConfirmContainer');
    const warningDiv = document.getElementById('manualMoveWarning');
    
    if (selectedUserToMove && selectedNewParent) {
        container.style.display = 'block';
        
        // Find user details from treeData if they exist, or check referrals count
        const downlineCount = selectedUserToMove.referrals || 0;
        if (downlineCount > 0) {
            warningDiv.style.display = 'block';
            warningDiv.innerHTML = `
                <strong>⚠️ Warning:</strong> This user has <strong>${downlineCount}</strong> referrals in their downline. 
                Moving them will shift their entire subtree. Their levels will be automatically recalculated relative to the new parent.
            `;
        } else {
...
async function handleManualPlacementSubmit() {
    const feedbackDiv = document.getElementById('manualPlacementFeedback');
    
    if (!selectedUserToMove) {
        alert('Please select a user to move');
        return;
    }
    ...
    if (selectedPositionSlot === null) {
        alert('Please select a free position slot');
        return;
    }
    
    const token = localStorage.getItem('token');
    const parentId = selectedNewParent._id === 'root' ? null : selectedNewParent._id;
    ...
    try {
        ...
        const response = await fetch(`${API}/admin/referral-tree/users/${selectedUserToMove._id}/tree-position`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
        ...
        if (!response.ok) {
            throw new Error(data.error || 'Failed to update tree position');
        }
        
        feedbackDiv.style.cssText = 'display: block; padding: 14px; border-radius: 6px; margin-top: 15px; background: #ecfdf5; color: #065f46; border-left: 4px solid #10b981; font-size: 14px; font-weight: 500;';
        feedbackDiv.innerHTML = `
            ✅ <strong>Placement Successful!</strong><br>
            Moved <strong>${data.summary.user.name}</strong> from Slot ${data.summary.oldPosition} to Slot ${data.summary.newPosition}.<br>
            Recalculated tree levels for <strong>${data.summary.descendantsRecalculated}</strong> descendants.
        `;
        
        selectedUserToMove = null;
        selectedNewParent = null;
        selectedPositionSlot = null;
        
        document.getElementById('searchUserToPlace').value = '';
        document.getElementById('searchParentUser').value = '';
        document.getElementById('placeAtRootToggle').checked = false;
        document.getElementById('parentSearchContainer').style.display = 'block';
        document.getElementById('selectedUserToPlaceDetails').style.display = 'none';
        document.getElementById('selectedParentDetails').style.display = 'none';
        document.getElementById('positionAndConfirmContainer').style.display = 'none';
        ...
        feedbackDiv.style.cssText = 'display: block; padding: 14px; border-radius: 6px; margin-top: 15px; background: #fef2f2; color: #991b1b; border-left: 4px solid #ef4444; font-size: 14px; font-weight: 500;';
        feedbackDiv.innerHTML = `❌ Error: ${err.message}`;
    }
}

// Initialize tool
initManualPlacementTool();