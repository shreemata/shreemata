...
async function buildLevelGroupedTree(rootUsers, currentDepth = 0, maxDepth = 20) {
    ...
    async function processUsersAtLevel(userIds, level) {
        ...
        .select("name email referralCode wallet treeLevel treePosition treeChildren treeParent referredBy createdAt directCommissionEarned treeCommissionEarned firstPurchaseDone isVirtual originalUser virtualEarningsBalancePaise virtualLifetimeEarningsPaise virtualClaimedEarningsPaise")
        .populate("originalUser", "name email referralCode");
        ...
        for (const user of users) {
            ...
            let ownerName = null;
            let virtualReferralNumber = null;

            if (isVirtual) {
                if (user.originalUser && typeof user.originalUser === 'object' && user.originalUser.name) {
                    ownerName = user.originalUser.name;
                } else if (user.originalUser) {
                    const owner = await User.findById(user.originalUser).select('name');
                    if (owner) ownerName = owner.name;
                }
                if (!ownerName && user.name) {
                    ownerName = user.name.split('-Virtual-')[0];
                }
                const match = (user.name || '').match(/-Virtual-(\d+)$/i);
                virtualReferralNumber = match ? parseInt(match[1]) : 1;
            }
            ...
            const userData = {
                ...
                wallet: isVirtual ? 0 : (user.wallet || 0),
                ...
                isVirtual: isVirtual,
                originalUser: user.originalUser ? (user.originalUser._id || user.originalUser) : null,
                ownerName: ownerName,
                virtualReferralNumber: virtualReferralNumber,
                virtualEarningsBalancePaise: user.virtualEarningsBalancePaise || 0,
                virtualLifetimeEarningsPaise: user.virtualLifetimeEarningsPaise || 0,
                virtualClaimedEarningsPaise: user.virtualClaimedEarningsPaise || 0
            };
...
async function buildCompleteTree(rootUsers, currentDepth = 0, maxDepth = 20) {
    ...
    for (const userId of rootUsers) {
        const user = await User.findById(userId)
            .select("name email referralCode wallet treeLevel treePosition treeChildren referredBy createdAt directCommissionEarned treeCommissionEarned firstPurchaseDone isVirtual originalUser virtualEarningsBalancePaise virtualLifetimeEarningsPaise virtualClaimedEarningsPaise")
...
router.get("/level/:level", authenticateToken, isAdmin, async (req, res) => {
    try {
        ...
        const usersAtLevel = await User.find({ 
            ...
            .select("name email referralCode wallet treePosition treeParent treeChildren referredBy createdAt directCommissionEarned treeCommissionEarned isVirtual originalUser virtualEarningsBalancePaise virtualLifetimeEarningsPaise virtualClaimedEarningsPaise")
...
/**
 * Helper to recursively check if targetUserId is a descendant of parentUser
 */
async function isDescendant(parentUser, targetUserId) {
    if (!parentUser || !parentUser.treeChildren || parentUser.treeChildren.length === 0) {
        return false;
    }
    
    const childIds = parentUser.treeChildren.map(id => id.toString());
    if (childIds.includes(targetUserId.toString())) {
        return true;
    }
    ...
    for (const childId of childIds) {
        ...
        if (childUser) {
            const found = await isDescendant(childUser, targetUserId);
...
/**
 * Helper to recursively update treeLevel for all descendants in a user's subtree
 */
async function updateDescendantLevels(user, currentLevel) {
    let count = 0;
    ...
    for (const childId of user.treeChildren) {
        ...
        if (child) {
            child.treeLevel = currentLevel + 1;
            await child.save();
            count++;
            count += await updateDescendantLevels(child, child.treeLevel);
        }
    }
    return count;
}
...
/**
 * GET /api/admin/referral-tree/parents/:parentId/slots
 * Get the occupation status of slots 0-4 under a parent user
 */
router.get("/parents/:parentId/slots", authenticateToken, isAdmin, async (req, res) => {
    try {
        ...
        const children = await User.find({ treeParent: targetParentId }).select('name email treePosition');
        
        const slots = [
            { position: 0, occupant: null },
        ...
        children.forEach(child => {
            if (child.treePosition >= 0 && child.treePosition <= 4) {
                slots[child.treePosition].occupant = {
                    id: child._id,
                    name: child.name,
                    email: child.email
                };
        ...
        console.error("Error fetching parent slots:", err);
        res.status(500).json({ error: "Server error fetching slots" });
    }
});

/**
 * PUT /api/admin/users/:userId/tree-position
 * Manually set/change a user's position in the tree
 */
router.put("/users/:userId/tree-position", authenticateToken, isAdmin, async (req, res) => {
    try {
        ...
        const user = await User.findById(userId);
        if (!user) {
            return res.status(404).json({ error: "User to be placed not found" });
        }
        
        // 2. Validate position
        const posNum = parseInt(position);
        if (isNaN(posNum) || posNum < 0 || posNum > 4) {
            return res.status(400).json({ error: "Position must be an integer between 0 and 4" });
        }
        ...
        if (newParentId) {
            // 3. Validate newParentId exists
            newParent = await User.findById(newParentId);
            if (!newParent) {
                return res.status(404).json({ error: "New parent user not found" });
            }
            
            // 4. Prevent circular placement
            if (newParentId.toString() === userId.toString()) {
                return res.status(400).json({ error: "Circular placement: User cannot be their own parent" });
            }
            
            const circular = await isDescendant(user, newParentId);
            if (circular) {
                return res.status(400).json({ error: "Circular placement: Target parent is a descendant of this user" });
            }
            
            // 5. Max 5 children check
            const isAlreadyChild = newParent.treeChildren.some(id => id.toString() === userId.toString());
            if (newParent.treeChildren.length >= 5 && !isAlreadyChild) {
                return res.status(400).json({ error: `Max children limit (5) reached for parent ${newParent.name}` });
            }
            
            // 6. Slot occupancy check
            const occupyingChild = await User.findOne({ treeParent: newParentId, treePosition: posNum });
            if (occupyingChild && occupyingChild._id.toString() !== userId.toString()) {
                return res.status(400).json({ error: `Position ${posNum} under ${newParent.name} is already occupied by ${occupyingChild.name}` });
            }
            
            newLevel = newParent.treeLevel + 1;
        } else {
            // Placing at root (no parent)
            const occupyingChild = await User.findOne({ treeParent: null, treePosition: posNum });
            if (occupyingChild && occupyingChild._id.toString() !== userId.toString()) {
                return res.status(400).json({ error: `Position ${posNum} at the root is already occupied by ${occupyingChild.name}` });
            }
        }
        
        const oldParentId = user.treeParent;
        const oldPosition = user.treePosition;
        
        // Remove from old parent's treeChildren if it exists
        if (oldParentId) {
            const oldParent = await User.findById(oldParentId);
            if (oldParent) {
                oldParent.treeChildren = oldParent.treeChildren.filter(id => id.toString() !== userId.toString());
                await oldParent.save();
            }
        }
        
        // Update user properties
        user.treeParent = newParentId ? newParent._id : null;
        user.treePosition = posNum;
        user.treeLevel = newLevel;
        await user.save();
        
        // Add to new parent's treeChildren if not null
        if (newParent) {
            const exists = newParent.treeChildren.some(id => id.toString() === userId.toString());
            if (!exists) {
                newParent.treeChildren.push(user._id);
                await newParent.save();
            }
        }
        
        // Recursively update treeLevel of all descendants
        const descendantsRecalculated = await updateDescendantLevels(user, newLevel);
        ...
        res.json({
            ...
            summary: {
                ...
                newPosition: posNum,
                newLevel: newLevel,
                descendantsRecalculated
            }
        });
        
    } catch (err) {
        console.error("Manual tree placement error:", err);
        res.status(500).json({ error: "Server error during manual tree placement" });
    }
});
